...
%     cfg_util('run',matlabbatch);
    clear matlabbatch;
    clearvars -except path Subj_list nt other_name_func other_name_anat func_fl_name anat_fl_name path_func path_anat sc_st rest_name sub_not_processed
...
excluded = zeros(1,length(Subj_list));
p = 1;
for subji = 1:length(Subj_list)                                % For loop on number od subjects
    ...
    if fd_max_trans(subji)>1.5 || fd_max_rotat(subji)>1.5 || fd_mean_trans(subji)>0.2 || fd_mean_rotat(subji)>0.2
        excluded(subji) = 1;
...
if nnz(excluded)
    ask = questdlg(['A total of ',num2str(nnz(excluded)),' Subjects were discarded out of ',num2str(length(Subj_list)),' subjects. Do you want to Proceed'],'Proceed?','Yes','No','No');
...
% output. For skull stripping you may either use AFNI or FSL
        if sc_st
            ...
            if ~(length(anat_image) == 1)                                                     % If there are more than 1 nifti files
                clear anat_image
...
%% Segmentation 
        ...
        matlabbatch{1}.spm.spatial.preproc.tissue(1).tpm = {'/mnt/d/iit_mandi/Research/spm12/spm12/tpm/TPM.nii,1'};
        matlabbatch{1}.spm.spatial.preproc.tissue(1).ngaus = 1;
        matlabbatch{1}.spm.spatial.preproc.tissue(1).native = [1 0];
        matlabbatch{1}.spm.spatial.preproc.tissue(1).warped = [0 0];
        matlabbatch{1}.spm.spatial.preproc.tissue(2).tpm = {'/mnt/d/iit_mandi/Research/spm12/spm12/tpm/TPM.nii,2'};
        matlabbatch{1}.spm.spatial.preproc.tissue(2).ngaus = 1;
        matlabbatch{1}.spm.spatial.preproc.tissue(2).native = [1 0];
        matlabbatch{1}.spm.spatial.preproc.tissue(2).warped = [0 0];
        matlabbatch{1}.spm.spatial.preproc.tissue(3).tpm = {'/mnt/d/iit_mandi/Research/spm12/spm12/tpm/TPM.nii,3'};
        matlabbatch{1}.spm.spatial.preproc.tissue(3).ngaus = 2;
        matlabbatch{1}.spm.spatial.preproc.tissue(3).native = [1 0];
        matlabbatch{1}.spm.spatial.preproc.tissue(3).warped = [0 0];
        matlabbatch{1}.spm.spatial.preproc.tissue(4).tpm = {'/mnt/d/iit_mandi/Research/spm12/spm12/tpm/TPM.nii,4'};
        matlabbatch{1}.spm.spatial.preproc.tissue(4).ngaus = 3;
        matlabbatch{1}.spm.spatial.preproc.tissue(4).native = [0 0];
        matlabbatch{1}.spm.spatial.preproc.tissue(4).warped = [0 0];
        matlabbatch{1}.spm.spatial.preproc.tissue(5).tpm = {'/mnt/d/iit_mandi/Research/spm12/spm12/tpm/TPM.nii,5'};
        matlabbatch{1}.spm.spatial.preproc.tissue(5).ngaus = 4;
        matlabbatch{1}.spm.spatial.preproc.tissue(5).native = [0 0];
        matlabbatch{1}.spm.spatial.preproc.tissue(5).warped = [0 0];
        matlabbatch{1}.spm.spatial.preproc.tissue(6).tpm = {'/mnt/d/iit_mandi/Research/spm12/spm12/tpm/TPM.nii,6'};
...
%% Nomralization
        ...
        matlabbatch{1}.spm.spatial.normalise.write.subj.def = {fullfile(anat_folder,['y_',anat_image.name])};%{'/mnt/d/NJIT/Research/Preprocessing/Practice/new/sub-10471/anat/y_sub-10471_T1w.nii'};
        clear pathlist
        for ii= 1:nt    % Number of timepoints
